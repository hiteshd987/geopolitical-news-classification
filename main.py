...
from scoring import calculate_risk_score, calculate_confidence
...
def process_single_article(idx, row):
    ...
    # We only send the top 3000 characters to the LLM. 
    ...
    if llm_data:
        ...
        print(f"[{idx}] ✅ SUCCESS: Risk {row['risk_score']}")
    else:
        row['event_labels'] = ""
        row['risk_score'] = 0.0
        row['confidence'] = "low" 
        row['rationale'] = "API Error."
        print(f"[{idx}] ❌ API FAILED.")
...
def main():
    ...
    fieldnames = list(articles[0].keys()) + ["event_labels", "risk_score", "confidence", "rationale", "keywords_detected"]
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        future_to_article = {executor.submit(process_single_article, idx+1, row): row for idx, row in enumerate(articles)}
        
        # Gather results as they finish
        for future in as_completed(future_to_article):
            results.append(future.result())

    # Sort results back into their original order (since async threads finish out of order based on article length)
    results = sorted(results, key=lambda x: articles.index({k: v for k, v in x.items() if k in articles[0]}))
